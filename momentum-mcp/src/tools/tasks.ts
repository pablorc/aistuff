...
export function registerTaskTools(server: McpServer, pool: Pool): void {
  server.tool(
    'create_task',
    'Create a new task',
    {
  ...
  server.tool(
    'get_task',
    'Retrieve a task by ID, including its tags and direct subtasks',
    ...
    async ({ id }) => {
      try {
        ...
        if (!rows[0]) return fail(new Error(`Task ${id} not found`));
        return ok(rows[0]);
      } catch (e) {
        return fail(e);
      }
  ...
  server.tool(
    'list_tasks',
    'List tasks with optional filters. Returns top-level tasks only (subtasks excluded).',
    {
  ...
  server.tool(
    'update_task',
    'Update mutable fields of an existing task. Only provided fields are changed.',
    {
      id: z.string().describe('Task UUID'),
    ...
    async ({ id, title, description, due_date, priority, tag_ids }) => {
      try {
        ...
        if (sets.length > 0) {
          ...
          vals.push(id);
          const { rows } = await pool.query(
            `UPDATE tasks SET ${sets.join(', ')} WHERE id = $${p} RETURNING *`,
            vals,
          );
          task = rows[0];
          if (!task) return fail(new Error(`Task ${id} not found`));
        }
        ...
        if (tag_ids !== undefined) {
          ...
          await Promise.all(
            tag_ids.map((tid) =>
              pool.query('INSERT INTO task_tags (task_id, tag_id) VALUES ($1, $2)', [id, tid]),
            ),
          );
        }
  ...
  server.tool(
    ...
    async ({ id }) => {
      try {
        ...
        if (!rowCount) return fail(new Error(`Task ${id} not found`));
  ...
  server.tool(
    'search_tasks',
    'Search active tasks by title or description (case-insensitive substring match)',
    {
  ...
  server.tool(
    'complete_task',
    'Mark a task as completed. For recurring tasks, the next occurrence is automatically created.',
    { id: z.string().describe('Task UUID') },
    async ({ id }) => {
      ...
      try {
        ...
        if (task.recurrence_rule_id) {
          ...
          const rule = rows[0] as RecurrenceRule;

          const nextDate = computeNextOccurrence(rule, task.due_date ?? new Date());
          if (nextDate) {
            const { rows: [nt] } = await client.query(
              `INSERT INTO tasks (title, description, priority, due_date, recurrence_rule_id)
               VALUES ($1, $2, $3, $4, $5)
               RETURNING *`,
              [task.title, task.description, task.priority, nextDate, task.recurrence_rule_id],
            );
            nextTask = nt as Record<string, unknown>;
            ...
            await Promise.all(
              tagRows.map(({ tag_id }: { tag_id: string }) =>
                client.query('INSERT INTO task_tags (task_id, tag_id) VALUES ($1, $2)', [
                  nt.id,
                  tag_id,
                ]),
              ),
  ...
  server.tool(
    'reopen_task',
    'Revert a completed task back to active status',
    { id: z.string().describe('Task UUID') },
    async ({ id }) => {
      try {
        const { rows: [task] } = await pool.query(
        ...
        if (!task) return fail(new Error(`Task ${id} not found or not completed`));
        return ok(task);
      } catch (e) {
        return fail(e);
      }
  ...
  server.tool(
    'add_subtask',
    'Add a child task under an existing parent (e.g. grocery items under a shopping trip)',
    {
    ...
    async ({ parent_task_id, title, description, due_date, priority }) => {
      try {
        const { rows: [task] } = await pool.query(
          `INSERT INTO tasks (title, description, due_date, priority, parent_task_id)
           VALUES ($1, $2, $3, $4, $5)
           RETURNING *`,
          [title, description ?? null, due_date ?? null, priority, parent_task_id],
        );
        return ok(task);
      } catch (e) {
        return fail(e);
