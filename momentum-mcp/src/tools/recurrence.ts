...
export function registerRecurrenceTools(server: McpServer, pool: Pool): void {
  server.tool(
    'create_recurring_task',
    'Create a task that repeats on a schedule. Each time it is completed a new occurrence is automatically generated.',
    {
      ...
      try {
        ...
        const { rows: [task] } = await client.query(
          `INSERT INTO tasks (title, description, priority, due_date, recurrence_rule_id)
           VALUES ($1, $2, $3, $4, $5)
           RETURNING *`,
          [title, description ?? null, priority, due_date, rule.id],
        );
  ...
  server.tool(
    ...
    'Preview the next N due dates for a recurring task without making any changes',
    {
    ...
    async ({ recurrence_rule_id, count }) => {
      try {
        ...
        const rule = rows[0] as RecurrenceRule | undefined;
        if (!rule) return fail(new Error(`Recurrence rule ${recurrence_rule_id} not found`));
        ...
        const dates: string[] = [];
        const sim: RecurrenceRule = { ...rule };
        let cursor: Date = latest[0]?.due_date ?? new Date();
